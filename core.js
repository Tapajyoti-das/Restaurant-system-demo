...
async function sbUpdateOrder(id, updates) {
  try {
    ...
    if (!count || count === 0) {
      ...
      const blocked = new Error('Update blocked by security policy — 0 rows affected (order may already be in a non-cancellable state, or you do not have permission to modify it).');
      console.error('sbUpdateOrder: 0 rows affected —', { id, updates, actualStatusRightNow: actualStatus });
