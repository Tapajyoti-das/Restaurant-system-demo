    ...
    if(qrTableNum) {
      ...
      if(matchedTable) {
        ...
        if(lockedType === 'Walk-in') {
          ...
          // Dine-In / VIP — try to find today's reservation for this table first
          ...
          if(activeReserv && activeReserv.name) {
            ...
            setTimeout(() => { try { updateCustNav(); } catch(e) {} }, 100);
          } else {
            // No reservation found — fall back to asking for name/WhatsApp
            // rather than getting stuck with no identity at all.
            window._qrNeedsQuickCapture = true;
          }
        }
      }
