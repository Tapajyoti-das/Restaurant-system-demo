...
async function sbUpdateOrder(id, updates) {
  try {
    ...
    if (!count || count === 0) {
      ...
      const blocked = new Error('Update blocked — 0 rows affected. [DIAGNOSTIC: order id='+id+', attempted update to status="'+updates.status+'", but row\'s ACTUAL current status right now is "'+actualStatus+'"]');
