...
// razorpay.com pe free account banao → Test Mode ON karo →
// Settings → API Keys → Generate Test Key → "Key ID" copy karke
// neeche paste karo (rzp_test_ se shuru hoti hai).
// NOTE: Sirf Key ID yahan daalo — Key SECRET kabhi code mein mat daalna!
// ============================================================
...
// Har admin/staff username ke liye ek Supabase Auth account hota hai
// (email = username@spicegarden.local — asli email ki zaroorat nahi).
// Login pe pehle Supabase Auth try hota hai; purane users ka account
// pehli login pe automatically ban jata hai (auto-migration).
// ============================================================
...
async function sgAuthSignUp(username, password) {
  // Secondary client use karte hain taaki current (admin) session clobber na ho
...
async function sbInsertOrder(order) {
  try {
    ...
    // NOTE: yahan .select() nahi lagana! Bina-login customer ko orders PADHNE
    // ki permission nahi hai (security), .select() lagane se poora insert fail
    // ho jata hai. Isi wajah se customer devices ke orders gayab ho rahe the.
...
async function sbUpdateOrder(id, updates) {
  try {
    const { error } = await _supabase
      ...
      .update(updates)
      .eq('id', id);
    // .select() nahi — customer (bina login) apna Pending order cancel karta hai,
    // usko orders padhne ki permission nahi hoti
