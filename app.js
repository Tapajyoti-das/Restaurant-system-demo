...
async function onConsumptionMatchInputs() {
  ...
  const totalWt   = (itemType==='rawmat' && totalWtEl) ? totalWtEl.value.trim() : '';
  resetConsumptionAutoFields();
  if(!itemType || !itemId) return;

  const usingTotalWeight = !!totalWt;
  if(!usingTotalWeight && !perWt) return; // nothing usable entered yet

  const displayLabel = usingTotalWeight ? totalWt : perWt;
  const allPurchases = await getPurchases();
  const today = new Date(); today.setHours(0,0,0,0);
  ...
  if(usingTotalWeight) {
    ...
    const wanted = parseWeightToBase(totalWt);
    if(wanted===null) { showConsumptionMatchStatus('invalid', totalWt); return; }
    const sorted = baseCandidates.slice().sort((a,b)=> new Date(a.date) - new Date(b.date));
    if(!sorted.length) { showConsumptionMatchStatus('none', totalWt); return; }
...
async function saveConsumption() {
  ...
  const usingTotalWeight = !!totalWeight;
