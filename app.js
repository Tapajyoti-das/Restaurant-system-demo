...
async function renderTrackRecords() {
  try {
    ...
    if(!rows.length) {
      tbody.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:30px;color:#aaa;">No usage record found' + (searchRaw ? ' for "'+searchRaw+'"' : '') + '</td></tr>';
      return;
    }
...
<div id="cancelAssignModal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.55);z-index:99999;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:16px;padding:28px;max-width:520px;width:90%;box-shadow:0 8px 40px rgba(0,0,0,0.18);">
    <div style="font-size:1.2rem;font-weight:800;margin-bottom:6px;">⚠️ Order was Cancelled after Preparing/Ready</div>
    <div style="font-size:0.85rem;color:var(--muted);margin-bottom:20px;">The ingredients have already been used. What would you like to do?</div>
    ...
    <div style="background:#e8f5e9;border-radius:12px;padding:16px;margin-bottom:14px;border:2px solid #a5d6a7;">
      <div style="font-weight:700;font-size:0.95rem;margin-bottom:10px;">✅ Option 1: Assign to another order</div>
      <div style="font-size:0.82rem;color:#2e7d32;margin-bottom:10px;">The ingredients will be used in another order — stock will be deducted for that order</div>
      ...
      <button onclick="confirmCancelAssign()" style="background:#2e7d32;color:#fff;border:none;padding:10px 20px;border-radius:8px;font-weight:700;cursor:pointer;width:100%;font-size:0.9rem;">✅ Assign to This Order → Stock Deduct</button>
    </div>
    
    <!-- Option 2: Manual waste entry -->
    <div style="background:#fce4ec;border-radius:12px;padding:16px;border:2px solid #ef9a9a;">
      <div style="font-weight:700;font-size:0.95rem;margin-bottom:10px;">🗑️ Option 2: Enter waste manually</div>
      <div style="font-size:0.82rem;color:#c62828;margin-bottom:10px;">The ingredients were wasted — stock will be reduced</div>
      <button onclick="confirmCancelWaste()" style="background:#c62828;color:#fff;border:none;padding:10px 20px;border-radius:8px;font-weight:700;cursor:pointer;width:100%;font-size:0.9rem;">🗑️ Enter Waste → Stock Minus</button>
    </div>
    
    <button onclick="closeCancelAssignModal()" style="margin-top:14px;width:100%;padding:8px;border:1px solid #ddd;border-radius:8px;background:#f5f5f5;color:var(--muted);cursor:pointer;font-family:inherit;">I will decide later</button>
  </div>
</div>
