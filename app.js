...
function submitOrder() {
  ...
  if(pay !== 'Cash on Delivery' && !window._sgPaymentDone) {
...
function buildDispositionRow(o, item, idx) {
  // An order can be cancelled at any stage — buttons are always available
...
function confirmReplace(orderId, idx, targetOrderId) {
  ...
  showToast('✅ Item assigned to Order #'+targetOrderId+' as a replacement!');
...
function dispositionSet(orderId, idx, kind) {
  ...
  const modal = document.getElementById('itemStatusModal'); if(modal) modal.innerHTML = buildItemStatusModalHtml(o);
  patchOrderRowStatus(o);
  showToast(kind==='employee' ? '👤 Marked as Employee Use!' : '🗑️ Marked as Waste!');
...
function confirmReasonAction() {
  ...
  if(type==='delete') {
    ...
    if(o){
      ...
      // If cancelled after Preparing/Ready, show the popup
...
function showCancelAssignModal(cancelledId, items) {
  ...
  if(eligibleOrders.length === 0) {
    sel.innerHTML = '<option value="">No eligible order available</option>';
  } else {
    // Group by status
...
async function confirmCancelAssign() {
  ...
  showToast('✅ Ingredients assigned to Order ' + targetId + ' — Stock deducted!');
  ...
  try {
    ...
    const doSave = async () => {
      try {
        ...
        // Diff > 0 means something was used before the purchase.
        // Look for an item/date/vendor/HRN match in the emergency entries
        // and push to Track Records with the original emergency date+time.
        try {
          ...
          for (const pur of newPurchases) {
            ...
            // .filter() — take ALL matching emergency entries for the same item
            ...
            // Create a separate Track Record row for each matched emergency entry
            // (with its own original date + time)
            allMatched.forEach(matched => {
              ...
              consumptions.unshift({
                ...
                itemId:           pur.itemId      || '',
                qty:              matched.qty,          // each entry's own qty
              ...
              // Mark the emergency entry as reconciled
            ...
            // Summary: how many entries were found, and the total qty
          ...
          if (totalLinked > 0) {
            ...
            showToast('🔗 ' + totalLinked + ' emergency entr' + (totalLinked>1?'ies':'y') + ' auto-linked by Purchase ID and added to Track Records!');
          }
...
async function renderTrackRecords() {
  try {
    ...
    if(!tbody) return;
    tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;padding:20px;color:#aaa;">⏳ Loading...</td></tr>';
    ...
    if(!rows.length) {
      tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;padding:30px;color:#aaa;">No usage record found' + (searchRaw ? ' for "'+searchRaw+'"' : '') + '</td></tr>';
      return;
    }
...
async function autoExpireCompanyProducts() {
  ...
  for(const p of purchases) {
    // Step 2: Check type — only solid/liquid (Company Products)
...
async function onConsumptionMatchInputs(source) {
  ...
  // Mode is decided by which field the person is actually typing into:
  // - typing into Total Weight directly → simple "take X from oldest batch" (FIFO)
  // - typing into Quantity / Per-Unit-Weight → match by pack/batch size, and
  //   mirror the equivalent total into the Total Weight box so it's always
  //   visible (marked as auto-filled, so it won't be mistaken for direct entry)
  const usingTotalWeight = (source === 'total') && !!totalWtTyped;
  ...
  const displayLabel = usingTotalWeight ? totalWtTyped : perWt;
  const allPurchases = await getPurchases();
  const today = new Date(); today.setHours(0,0,0,0);
  ...
  if(usingTotalWeight) {
    ...
    const wanted = parseWeightToBase(totalWtTyped);
    if(wanted===null) { showConsumptionMatchStatus('invalid', totalWtTyped); return; }
    const sorted = baseCandidates.slice().sort((a,b)=> new Date(a.date) - new Date(b.date));
    if(!sorted.length) { showConsumptionMatchStatus('none', totalWtTyped); return; }
...
async function payTimingChoice(now) {
  ...
  if(linkedOrderId) {
    ...
    await sbUpdateOrder(linkedOrderId, { items, subtotal, gstAmt, total, grandTotal: total }).catch(e=>console.error('bookTableOrder update fail:',e));
    ...
    await sbInsertOrder(order).catch(e=>console.error('bookTableOrder create fail:',e));
  ...
  if(now) {
    ...
    showToast('✅ Order sent to kitchen! Generate the bill whenever the customer is ready to pay.');
  }
...
async function confirmPayment() {
  ...
  if(linkedOrderId) {
    await sbUpdateOrder(linkedOrderId, { pay:payMode||'Cash', paid:true }).catch(e => console.error('Table order payment update failed:', e));
    ...
    if(typeof orders !== 'undefined') orders.unshift(order);
    await sbInsertOrder(order).catch(e=>console.error('confirmPayment fallback order create fail:',e));
    try { autoAssignCookToOrder(linkedOrderId); } catch(e) {}
  }
  ...
  renderFloorPlan();
  showToast('✅ Payment complete! Please update the table status when ready.');
...
function showEventsTab(tab, btn) {
  try {
  ['list','zones','blocklist','reports'].forEach(t=>{
...
function addEvMenuItemFromMain() {
  ...
  const item = menuItems.find(m=>String(m.id)===String(id));
  if(!item) return;
  evMenuItemsTemp.push({ itemName:item.name, sourceItemId:item.id, isNewItem:false, price:item.price });
  sel.value='';
  renderEvMenuItemsList();
}
...
async function saveEvent() {
  ...
  const { error } = await _supabase.from('sg_events').upsert([event], { onConflict:'id' });
...
async function checkEventReminders() {
  ...
  const due = events.filter(ev=>{
    if(ev.reminderSent || !ev.eventDate || !ev.eventTime) return false;
    ...
    return hoursLeft > 0 && hoursLeft <= 5;
  });
...
async function openEventDetail(eventId) {
  ...
  document.getElementById('evDetailContent').innerHTML = `
    ...
    <div class="detail-row"><span class="detail-key">Guest List</span><span class="detail-val">${(ev.guestList||[]).filter(g=>!g.isExtra).length} named guest(s) added${(ev.guestList||[]).filter(g=>g.isExtra).length?' + '+(ev.guestList||[]).filter(g=>g.isExtra).length+' extra guest(s)':''}</span></div>
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      ...
      <button class="act-btn" onclick="addGuestToEvent('${eventId}')" style="background:#e3f2fd;color:#1565c0;">📋 Add Guest (List)</button>
...
async function addGuestToEvent(eventId) {
  ...
  if(!name) return;
  const phone = prompt('Guest phone (optional):','')||'';
  const guests = (ev.guestList||[]);
  guests.push({guestName:name, guestPhone:phone, isExtra:false, addedAt:new Date().toISOString()});
  const { error } = await _supabase.from('sg_events').update({guestList:guests}).eq('id', eventId);
  if(error) { console.error('addGuestToEvent error:', error); return; }
  showToast('✅ Guest added: '+name);
  openEventDetail(eventId);
  } catch(_e) { console.error("[SG] addGuestToEvent:", _e); }
}
