...
function addItemsToTable() {
  ...
  // Replace this table's active order entirely with the corrected cart,
  // instead of stacking a new entry on top — this lets a wrongly-selected
  // item be properly reduced/removed on the next "Add Items" pass.
...
async function confirmPayment() {
  ...
  const allItems = activeOrds.flatMap(o=>o.items);
  const items = allItems.map(i=>({ name:i.name, qty:i.qty, price:i.price }));
  const subtotal = allItems.reduce((s,i)=>s+i.price*i.qty, 0);
  const gstAmt = Math.round(subtotal * getGSTRate()/100);
  const total = subtotal + gstAmt;
  const combinedNote = activeOrds.map(o=>o.note).filter(Boolean).join(' | ');

  const orderId='ORD'+Date.now().toString(36).toUpperCase()+Math.floor(Math.random()*1296).toString(36).toUpperCase().padStart(2,'0');
  const order={
    id:orderId, name:'Walk-in Guest', mobile:'', whatsapp:'', email:'', address:'',
    type:'Walk-In', pay:payMode||'Cash', note:combinedNote, items, subtotal, gstAmt, total,
    status:'Served', time:new Date().toLocaleString('en-IN'),
    tableNum: t ? t.num : '', tableId: tableId,
    grandTotal: total, payment_id:'', paid:true
  };
  if(typeof orders !== 'undefined') orders.unshift(order);
  await sbInsertOrder(order).catch(e => console.error('Table order save failed:', e));

  tblOrders.forEach(o=>{
    if(o.tableId===tableId && o.status==='active') {
      o.status='paid'; o.payMode=payMode; o.paidAt=new Date().toLocaleString('en-IN'); o.linkedOrderId=orderId;
    }
  });
  setTblOrders(tblOrders);
  ...
  renderFloorPlan();
  showToast('✅ Payment complete! Order #'+orderId+' saved. Table set to Cleaning.');
