...
// Payment success => window._sgPaymentDone set karke submitOrder dobara
// ============================================================
function openRazorpayCheckout(amount, cust) {
  try {
    if(typeof Razorpay === 'undefined') {
      showToast('⚠️ Payment system load nahi hua — internet check karke page refresh karo', 'error');
      return;
    }
    if(typeof RAZORPAY_KEY_ID === 'undefined' || !RAZORPAY_KEY_ID || RAZORPAY_KEY_ID.indexOf('PASTE') !== -1) {
      showToast('⚠️ Razorpay Test Key set nahi hui hai — core.js mein RAZORPAY_KEY_ID daalo. Tab tak Cash select karo.', 'error');
      return;
    }
    const rzp = new Razorpay({
      ...
      modal: { ondismiss: function() { showToast('Payment cancel hua — order place nahi hua', 'error'); } }
    });
    ...
    showToast('Payment window kholne mein problem aayi', 'error');
  }
}
...
function submitOrder() {
  ...
  // UPI/Card/Net Banking select kiya hai toh pehle payment popup khulega;
  // payment SUCCESS hone par hi order banega. Cash pe seedha order.
  // ============================================================
  ...
  if(pay !== 'Cash on Delivery' && !window._sgPaymentDone) {
    ...
    return; // payment complete hone par submitOrder apne aap dobara chalega
  }
...
function restoreAdminSession() {
  try {
    ...
    // Security fix: Supabase Auth token bhi hona chahiye — warna database
    // khaali jawab dega (purane upgrade-se-pehle wale sessions ka masla).
    // Token nahi mila toh purana session hatao aur fresh login mangao.
    _supabase.auth.getSession().then(({ data }) => {
      if(!data || !data.session) {
        console.warn('[SG] Purana session bina Supabase token ke — fresh login chahiye');
        sessionStorage.removeItem('sg_session');
        try { showToast('🔐 Session expire ho gaya — dobara login karo', 'error'); } catch(e) {}
