...
async function doCancelMyOrder(orderId, reason) {
  ...
  // Cancellation now goes through a server-side Edge Function (service-role)
  // instead of a direct client-side RLS-restricted UPDATE. The direct-UPDATE
  // path was returning HTTP-204 (success) while genuinely affecting 0 rows,
  // for reasons that checked out fine at every RLS/policy/grant level we
  // could inspect — the Edge Function does the identical validation
  // (must be Pending/Confirm/Preparing/Ready) but performs the actual write
  // with elevated privileges, giving a reliable, unambiguous result.
  let result;
  try {
    const resp = await fetch('https://dbdstcfgfetvrawzkgoh.supabase.co/functions/v1/cancel-order', {
      method: 'POST',
      headers: {
    ...
    if(!resp.ok) {
      console.error('[SG] cancel-order function error:', result);
      const msg = result && result.error ? result.error : ('Server returned '+resp.status);
      if(result && result.currentStatus) {
        o.status = result.currentStatus; // sync stale local copy
        renderMyOrders();
      }
      showToast('⚠️ Could not cancel your order (' + msg + ').','error');
      return;
    }
  } catch(fetchErr) {
    console.error('[SG] cancel-order fetch exception:', fetchErr);
    showToast('⚠️ Could not reach the server to cancel your order. Please check your connection and try again.','error');
    return;
  }
