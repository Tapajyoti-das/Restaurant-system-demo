...
async function renderKPI() {
  ...
  const purchases = (await invGet('purchases')) || [];
  ...
  // Waste % — compare waste against the value of CURRENT stock (not just purchases
  // made in this exact period), since wasted items may have been bought long ago.
  // For each item, find its most recent purchase price and value the current qty by it.
  const latestPriceByItem = {};
  purchases.forEach(p => {
    const key = p.itemId;
    if (!key) return;
    const d = parseAnyDate(p);
    if (!d) return;
    if (!latestPriceByItem[key] || d > latestPriceByItem[key].date) {
      latestPriceByItem[key] = { date: d, price: parseFloat(p.price) || 0 };
    }
  });
  const totalStockValue = stock.reduce((s,x) => {
    const info = latestPriceByItem[x.itemId];
