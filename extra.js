...
async function saveConsumEntryEdit(id) {
  try {
    ...
    const updatePayload = isEmpUse
      ? { qty, unitcost, totalcost, consumedby: nameVal }
      : { qty, unitcost, totalcost, servedby: nameVal };
...
async function openEndOfDay(entryId) {
  ...
  if(!entry.name && entry.batch) {
    showToast('⚠️ This entry only has a Batch/HRN (no item name yet). It will auto-link once a matching Purchase with the same HRN is entered — no manual reconciliation needed for it.', 'error');
    return;
  }
...
(Remaining: ${entry.qty - parseFloat(usedQty||0)} ${entry.unit})`);
  ...
  // Actually move the "Used" qty into Main Stock + Track Records (this was
  // previously missing — the entry got marked reconciled but stock/consumption
  // was never actually updated, even though the button claimed "Used→Stock").
  if(used > 0) {
    const products = await getProducts();
    const rawmats  = await getRawmats();
    ...
    if(matched) {
      await updateStock(itemType, matched.id, used, 'add');
      ...
      consumptions.unshift({
        ...
        purpose:    entry.reason   || 'Emergency Usage (End of Day)',
        recordedBy: entry.takenBy  || '',
        approvedBy: currentUser?.name || '—',
        handoverTo: entry.takenBy  || '—',
        source:     'emergency_eod',
      ...
      showToast('⚠️ Used qty saved, but no matching Product/Raw Material named "'+entry.name+'" was found — Main Stock not updated. Please add it there manually.','error');
    }
  }
