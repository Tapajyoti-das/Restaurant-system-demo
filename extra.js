...
let _consumLogCols = {
  date: true, time: true, orderid: true, replaceorderid: true, consumedby: true, purchaseids: true,
  dishname: true, ingredientname: true, ingredienttype: true,
  qty: true, unit: true, unitcost: true, totalcost: true, servedby: true
};

// Cache of last-fetched consumption rows (used by Edit modal so we don't refetch)
let _consumLogCache = [];
...
function openConsumLogColManager() {
  ...
  const cols = [
    ...
    {key:'replaceorderid', label:'Replace Order ID'},
    {key:'consumedby',     label:'Consumption by EMP'},
...
function saveConsumLogCols() {
  const keys = ['date','time','orderid','replaceorderid','consumedby','purchaseids','dishname','ingredientname','ingredienttype','qty','unit','unitcost','totalcost','servedby'];
  keys.forEach(k => {
...
async function renderConsumptionLog() {
  ...
  tbody.innerHTML = '<tr><td colspan="16" style="text-align:center;padding:20px;color:var(--muted);">⏳ Loading...</td></tr>';
  ...
  if(error) {
    ...
    tbody.innerHTML = '<tr><td colspan="16" style="text-align:center;padding:20px;color:#e74c3c;">Error: ' + (error.message||'Unknown') + '</td></tr>';
    return;
  }
  if(!data || !data.length) {
    tbody.innerHTML = '<tr><td colspan="16" style="text-align:center;padding:20px;color:var(--muted);">No consumption record found. (data length: 0)</td></tr>';
    return;
  }

  // Cache for the Edit modal to read from
  _consumLogCache = data;
  ...
  // Date range validation
  if(fromDate && toDate && new Date(fromDate) > new Date(toDate)) {
    tbody.innerHTML = '<tr><td colspan="16" style="text-align:center;padding:20px;color:#e74c3c;">⚠️ The start date is after the end date — fix the date range</td></tr>';
  ...
  if(!filtered.length) {
    tbody.innerHTML = '<tr><td colspan="16" style="text-align:center;padding:20px;color:var(--muted);">No record found for this filter</td></tr>';
    const summary = document.getElementById('consumLogSummary');
    if(summary) summary.style.display = 'none';
    return;
  }
  ...
  if(thead) {
    const colDefs = [
      ...
      {key:'replaceorderid', label:'Replace Order ID',  align:'left'},
      {key:'consumedby',     label:'Consumption by EMP',align:'left'},
    ...
    thead.innerHTML = `<tr style="background:#f9f7f5;"><th style="position:sticky;left:0;z-index:4;background:#f9f7f5;border-right:2px solid #e8d5c8;text-align:center;padding:10px 8px;">S.No.</th>${visibleCols.map(c =>
      `<th style="padding:10px 12px;text-align:${c.align};border-bottom:2px solid #e8d5c8;">${c.label}</th>`
    ).join('')}<th style="padding:10px 12px;text-align:center;border-bottom:2px solid #e8d5c8;">Action</th></tr>`;
  }
  ...
  tbody.innerHTML = filtered.map((r, _idx) => {
    ...
    // Replace Order ID — this consumption entry's order is the one that actually
    // consumed the dish (target order). If that item was received via a 🔁 Replace,
    // its own record carries replacedFromOrder (the source/cancelled order's id).
    const _thisOrderId = r.orderid || r.orderId || '';
    const _thisOrder = (typeof orders !== 'undefined') ? orders.find(o => o.id === _thisOrderId) : null;
    const _dishNm = (r.dishname || r.dishName || '').toLowerCase().trim();
    const _receivedItem = _thisOrder?.items?.find(it =>
      it.replacedFromOrder &&
      String(it.name||'').replace(/\s*\([^)]*\)\s*$/,'').trim().toLowerCase() === _dishNm.replace(/\s*\([^)]*\)\s*$/,'').trim()
    );
    const _replaceOrderId = _receivedItem?.replacedFromOrder || '';
    ...
    const allCells = {
      ...
      replaceorderid: `<td style="padding:8px 12px;">${_replaceOrderId ? '<span style="color:#1565c0;font-weight:700;font-size:0.8rem;">🔁 '+_replaceOrderId+'</span>' : '<span style="color:var(--muted);">—</span>'}</td>`,
      consumedby:     `<td style="padding:8px 12px;">${r.consumedby ? '<span style="color:#6a1b9a;font-weight:700;font-size:0.8rem;">👤 '+r.consumedby+'</span>' : '<span style="color:var(--muted);">—</span>'}</td>`,
      ...
      unitcost:       `<td style="padding:8px 8px;text-align:right;">${r.costpending ? '<span style="color:#e65100;font-weight:700;font-size:0.76rem;background:#fff3e0;padding:2px 8px;border-radius:10px;" title="Awaiting a matching Purchase entry">⏳ Pending</span>' : '₹'+unitcost.toFixed(2)}</td>`,
      totalcost:      `<td style="padding:8px 8px;text-align:right;font-weight:700;color:var(--fire);">${r.costpending ? '<span style="color:#e65100;font-weight:700;font-size:0.76rem;background:#fff3e0;padding:2px 8px;border-radius:10px;" title="Awaiting a matching Purchase entry">⏳ Pending</span>' : '₹'+totalcost.toFixed(2)}</td>`,
    ...
    const isAdmin = currentUser?.role === 'Admin';
    const _rowId = r.id || '';
    const _actionTd = isAdmin
      ? `<td style="padding:8px 8px;text-align:center;"><button onclick="openEditConsumEntry('${_rowId}')" style="background:#e8f0fe;color:#1565c0;border:2px solid #90caf9;padding:5px 10px;border-radius:7px;font-weight:700;cursor:pointer;font-size:0.76rem;">✏️ Edit</button></td>`
      : `<td style="padding:8px 8px;text-align:center;color:var(--muted);font-size:0.76rem;">—</td>`;
    return `<tr style="border-bottom:1px solid #f0f0f0;">${_snoTd}${visibleKeys.map(k => allCells[k]).join('')}${_actionTd}</tr>`;
  }).join('');
...
// ===== EDIT CONSUMPTION LOG ENTRY =====

function openEditConsumEntry(id) {
  if(currentUser?.role !== 'Admin') { _sgErrorToast?.('Only Admin can edit consumption entries'); return; }

  const row = _consumLogCache.find(r => String(r.id) === String(id));
  if(!row) { _sgErrorToast?.('Entry not found — please refresh and try again'); return; }

  document.getElementById('editConsumModal')?.remove();

  const qty        = parseFloat(row.qty || 0);
  const unitcost    = parseFloat(row.unitcost || row.unitCost || 0);
  const isEmpUse    = row.reason === 'employee_use';
  const nameLabel   = isEmpUse ? 'Consumption by EMP (Employee Name)' : 'Served By';
  const nameValue   = isEmpUse ? (row.consumedby || '') : (row.servedby || row.servedBy || '');

  const modal = document.createElement('div');
  modal.id = 'editConsumModal';
  modal.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.55);z-index:99999;display:flex;align-items:center;justify-content:center;';
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:380px;width:90%;max-height:85vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,0.2);">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
        <h3 style="margin:0;font-size:1rem;font-weight:800;">✏️ Edit Consumption Entry</h3>
        <button onclick="document.getElementById('editConsumModal').remove()"
          style="background:none;border:none;font-size:1.4rem;cursor:pointer;color:#666;line-height:1;">✕</button>
      </div>
      <div style="font-size:0.8rem;color:var(--muted);margin-bottom:14px;">
        ${row.dishname || row.dishName || '—'} → ${row.ingredientname || row.ingredientName || '—'}
      </div>

      <label style="display:block;font-size:0.82rem;font-weight:700;margin-bottom:4px;">Qty (${row.unit || ''})</label>
      <input type="number" id="editConsumQty" value="${qty}" step="0.001"
        style="width:100%;padding:9px 10px;border:2px solid #e8d5c8;border-radius:8px;font-family:inherit;margin-bottom:12px;">

      <label style="display:block;font-size:0.82rem;font-weight:700;margin-bottom:4px;">Unit Cost (₹)</label>
      <input type="number" id="editConsumUnitCost" value="${unitcost}" step="0.01"
        style="width:100%;padding:9px 10px;border:2px solid #e8d5c8;border-radius:8px;font-family:inherit;margin-bottom:12px;">

      <label style="display:block;font-size:0.82rem;font-weight:700;margin-bottom:4px;">${nameLabel}</label>
      <input type="text" id="editConsumName" data-is-emp-use="${isEmpUse ? '1' : '0'}" value="${nameValue}"
        style="width:100%;padding:9px 10px;border:2px solid #e8d5c8;border-radius:8px;font-family:inherit;margin-bottom:16px;">
      ...
      <div style="display:flex;gap:8px;">
        ...
        <button onclick="saveConsumEntryEdit('${id}')"
          style="flex:2;padding:10px;background:#E8400C;color:#fff;border:none;border-radius:8px;font-weight:700;cursor:pointer;font-family:inherit;">
          ✅ Save Changes
        </button>
      </div>
...
async function saveConsumEntryEdit(id) {
  try {
    if(currentUser?.role !== 'Admin') { _sgErrorToast?.('Only Admin can edit consumption entries'); return; }

    const qty      = parseFloat(document.getElementById('editConsumQty')?.value);
    const unitcost = parseFloat(document.getElementById('editConsumUnitCost')?.value);
    const nameEl   = document.getElementById('editConsumName');
    const nameVal  = nameEl?.value?.trim() || '';
    const isEmpUse = nameEl?.dataset?.isEmpUse === '1';

    if(isNaN(qty) || qty < 0) { _sgErrorToast?.('Enter a valid quantity'); return; }
    if(isNaN(unitcost) || unitcost < 0) { _sgErrorToast?.('Enter a valid unit cost'); return; }

    const totalcost = qty * unitcost;
    const updatePayload = isEmpUse
      ? { qty, unitcost, totalcost, consumedby: nameVal, costpending: false, pendingqty: 0 }
      : { qty, unitcost, totalcost, servedby: nameVal, costpending: false, pendingqty: 0 };
    ...
    if(error) { _sgErrorToast?.('Update failed: ' + (error.message || 'Unknown error')); return; }

    document.getElementById('editConsumModal')?.remove();
    await renderConsumptionLog();
    _sgErrorToast?.('✅ Consumption entry updated');
  } catch(e) {
    console.error('[SG] saveConsumEntryEdit:', e);
    _sgErrorToast?.('Something went wrong while saving');
  }
}
  ...
  div.innerHTML = keys.map(dishId => {
    ...
    return `<div style="background:#f9f7f5;border-radius:10px;padding:14px;margin-bottom:10px;border:1px solid #e8d5c8;">
      ...
      <div style="font-size:0.82rem;color:var(--muted);">
        ${recipe.map(r=>{
          const qtyLabel = (r.qty && typeof r.qty === 'object')
            ? ['Large','Medium','Small'].filter(s=>r.qty[s]!==undefined).map(s=>s[0]+':'+r.qty[s]).join(' ')
            : r.qty;
          return `<span style="background:#fff;border:1px solid #e8d5c8;border-radius:6px;padding:2px 8px;margin:2px;display:inline-block;">
          ${escHtml(r.name)}: ${qtyLabel} ${r.unit}
        </span>`;
        }).join('')}
      </div>
    </div>`;
...
async function renderRecipeIngredients(existing=[]) {
  ...
  div.innerHTML = (existing.length ? existing : [{}]).map((ing, idx) => buildIngRowHtml(idx, ing, itemOpts)).join('');
  } catch(_e) { console.error("[SG] renderRecipeIngredients:", _e); }
}

// Builds one ingredient row — either a single Qty field (same for every size) or
// three size-specific Qty fields (Large/Medium/Small), toggled by a checkbox.
function buildIngRowHtml(idx, ing, itemOpts) {
  const isSized = ing.qty && typeof ing.qty === 'object';
  const L = isSized ? (ing.qty.Large  ?? '') : '';
  const M = isSized ? (ing.qty.Medium ?? '') : '';
  const S = isSized ? (ing.qty.Small  ?? '') : '';
  const singleVal = isSized ? '' : (ing.qty ?? '');

  const qtyFieldsHtml = `
    <div id="ingQtySimple_${idx}" style="display:${isSized?'none':'block'};">
      <input type="number" placeholder="Qty" value="${singleVal}" min="0" step="0.01"
        id="ingQty_${idx}" style="width:100%;padding:8px;border:2px solid #e8d5c8;border-radius:8px;font-family:inherit;">
    </div>
    <div id="ingQtySized_${idx}" style="display:${isSized?'grid':'none'};grid-template-columns:1fr 1fr 1fr;gap:4px;">
      <input type="number" placeholder="Large" value="${L}" min="0" step="0.01" id="ingQtyLarge_${idx}"
        style="padding:6px;border:2px solid #a5d6a7;border-radius:6px;font-family:inherit;font-size:0.78rem;" title="Qty for Large size">
      <input type="number" placeholder="Medium" value="${M}" min="0" step="0.01" id="ingQtyMedium_${idx}"
        style="padding:6px;border:2px solid #ffb74d;border-radius:6px;font-family:inherit;font-size:0.78rem;" title="Qty for Medium size">
      <input type="number" placeholder="Small" value="${S}" min="0" step="0.01" id="ingQtySmall_${idx}"
        style="padding:6px;border:2px solid #ef9a9a;border-radius:6px;font-family:inherit;font-size:0.78rem;" title="Qty for Small size">
    </div>`;

  return `
    <div style="border:1px solid #eee;border-radius:8px;padding:8px;margin-bottom:8px;" id="ingRow_${idx}">
      <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:start;">
        <select style="padding:8px;border:2px solid #e8d5c8;border-radius:8px;font-family:inherit;" id="ingName_${idx}">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-top:6px;">
        <select id="ingUnit_${idx}" style="padding:6px 8px;border:2px solid #e8d5c8;border-radius:8px;font-family:inherit;font-size:0.8rem;">
        ...
        <label style="font-size:0.76rem;color:var(--muted);display:flex;align-items:center;gap:5px;cursor:pointer;">
          <input type="checkbox" id="ingSizeToggle_${idx}" ${isSized?'checked':''} onchange="toggleIngSizeMode(${idx})">
          Different qty per size (Large/Medium/Small)
        </label>
      </div>
    </div>`;
}

function toggleIngSizeMode(idx) {
  try {
  const isSized = document.getElementById('ingSizeToggle_'+idx)?.checked;
  const simpleDiv = document.getElementById('ingQtySimple_'+idx);
  const sizedDiv  = document.getElementById('ingQtySized_'+idx);
  if(simpleDiv) simpleDiv.style.display = isSized ? 'none' : 'block';
  if(sizedDiv)  sizedDiv.style.display  = isSized ? 'grid' : 'none';
  } catch(_e) { console.error("[SG] toggleIngSizeMode:", _e); }
}
...
async function addRecipeIngredient() {
  ...
  newRow.innerHTML = buildIngRowHtml(idx, {}, itemOpts);
  div.appendChild(newRow.firstElementChild);
...
async function saveRecipe() {
  ...
  rows.forEach((row, i) => {
    ...
    const isSized = document.getElementById('ingSizeToggle_' + i)?.checked;
    if(!nameEl || !nameEl.value) return;

    if(isSized) {
      const lEl = document.getElementById('ingQtyLarge_' + i);
      const mEl = document.getElementById('ingQtyMedium_' + i);
      const sEl = document.getElementById('ingQtySmall_' + i);
      const qtyObj = {};
      if(lEl && lEl.value !== '') qtyObj.Large  = parseFloat(lEl.value);
      if(mEl && mEl.value !== '') qtyObj.Medium = parseFloat(mEl.value);
      if(sEl && sEl.value !== '') qtyObj.Small  = parseFloat(sEl.value);
      if(Object.keys(qtyObj).length) {
        ingredients.push({ name: nameEl.value, qty: qtyObj, unit: unitEl ? unitEl.value : 'piece' });
      }
    } else {
      const qtyEl = document.getElementById('ingQty_' + i);
      if(qtyEl && qtyEl.value) {
        ingredients.push({ name: nameEl.value, qty: parseFloat(qtyEl.value), unit: unitEl ? unitEl.value : 'piece' });
      }
    }
  });
...
async function getIngredientCost(itemId) {
  ...
  // Fallback: calculate from price (per unit) ÷ unitWt (per unit weight) —
  // only used for legacy purchase rows that don't have basePricePerUnit saved
  const perGramCosts = [];
  itemPurchases.forEach(p => {
    const price  = parseFloat(p.price||0); // this is "price PER UNIT", not total
    const unitWt = p.unitWt || p["unitWt"] || '';
    const grams  = (typeof parseWtToGrams === 'function') ? parseWtToGrams(unitWt) : 0;
    if(grams > 0) perGramCosts.push(price / grams);
  });
  if(!perGramCosts.length) return 0;
  const avg = perGramCosts.reduce((a,b)=>a+b, 0) / perGramCosts.length;
  return Math.round(avg * 10000) / 10000;
  } catch(e) { return 0; }
}
...
async function getFIFOCost(itemId, neededQty) {
  ...
  if(error || !purs || !purs.length) {
    // No purchase batches at all backing this item yet — fully pending
    return { unitCost: null, totalCost: null, purchaseIds: [], usedPurchases: [], costPending: true, pendingQty: neededQty, partialCost: 0 };
  }
  ...
  if(remainingNeeded > 0) {
    // Some (or all) of the needed qty has no real purchase batch behind it yet
    // (e.g. Emergency-sourced stock with no formal Purchase logged). Don't
    // guess with an average cost — leave the whole entry pending until a
    // matching Purchase comes in and can be properly FIFO-allocated.
    return { unitCost: null, totalCost: null, purchaseIds, usedPurchases, costPending: true, pendingQty: remainingNeeded, partialCost: totalCost };
  }

  const unitCost = neededQty > 0 ? Math.round((totalCost / neededQty) * 10000) / 10000 : 0;
  totalCost = Math.round(totalCost * 100) / 100;
  return { unitCost, totalCost, purchaseIds, usedPurchases, costPending: false, pendingQty: 0, partialCost: 0 };
  } catch(e) {
...
// Resolve an ingredient's quantity for a specific order item — supports both the
// legacy format (ing.qty is a plain number, same regardless of size) and the new
// size-aware format (ing.qty is {Large, Medium, Small}, set per ingredient in Recipe Setup)
function resolveIngQty(ing, item) {
  const q = ing.qty;
  if(q && typeof q === 'object') {
    const sizeLabel = item.sizeLabel || 'Large';
    const direct = q[sizeLabel];
    if(direct !== undefined && direct !== null && direct !== '') return parseFloat(direct) || 0;
    // Size not set for this item's chosen size — fall back to whichever is filled in
    const fallback = q.Large ?? q.Medium ?? q.Small;
    return parseFloat(fallback) || 0;
  }
  return parseFloat(q) || 0;
}

async function autoDeductStockForOrder(orderItems, orderId, servedBy, reason) {
  ...
  // Duplicate check — if this order (or this specific item, for employee_use) has
  // already been consumed, skip it
  try {
    let dupQuery = _supabase.from('inv_consumptions').select('id').eq('orderid', orderId);
    if(reason === 'employee_use') {
      dupQuery = dupQuery.eq('reason', 'employee_use').eq('dishname', orderItems[0]?.name || '—');
    } else {
      dupQuery = dupQuery.or('reason.eq.served,reason.is.null');
    }
    const { data: existing, error: dupErr } = await dupQuery.limit(1);
    if(!dupErr && existing && existing.length > 0) {
      console.log('[SG] autoDeduct skipped — already consumed for order:', orderId, reason);
      return;
    }
  ...
  function getRecipe(item) {
    ...
    // Name se id dhundo — strip size suffix e.g. "Butter Chicken (Large)" → "Butter Chicken",
    // since menu items are keyed by base name without the size variant
    if(item.name) {
      const baseName = String(item.name).replace(/\s*\([^)]*\)\s*$/,'').trim().toLowerCase();
      const fullName = String(item.name).toLowerCase().trim();
      const mappedId = nameToId[fullName] || nameToId[baseName];
  ...
  for(const item of orderItems) {
    ...
    for(const ing of recipe) {
      ...
      const ingType = prod ? (prod.type||'company') : 'rawmat';
      const stockKey = ingType + '_' + ingItem.id;
      const stockEntry = stockMap[stockKey];
      if(!stockEntry) continue;

      const deductQty = resolveIngQty(ing, item) * (parseFloat(item.qty)||1);
      ...
      const { unitCost, totalCost, purchaseIds, usedPurchases, costPending, pendingQty, partialCost } = await getFIFOCost(ingItem.id, baseDeductQty);
      ...
      consumptionLogs.push({
        ...
        partialcost:     partialCost || 0,
        totalbaseqty:    baseDeductQty,
        date:            dateStr,
        time:            timeStr,
        servedby:        reason === 'employee_use' ? '' : (servedBy || 'Unknown'),
        consumedby:      reason === 'employee_use' ? (servedBy || 'Unknown') : '',
        reason:          reason,
...
async function renderExpiryProducts() {
  try {
    ...
    // Products (Company Product type) with future expiry — NOT expired yet
    const rows = purchases
      .filter(p => p.itemType === 'company' || p.itemType === 'solid' || p.itemType === 'liquid' || p.itemType === 'product')
...
function renderEmergencyRow(e, i, allEntries, done=false) {
  try {
  const displayName = e.name || (e.batch ? ('Batch #' + e.batch + ' (item pending — will auto-fill from matching Purchase)') : 'Unnamed item');
  const awaitingLink = !e.name && e.batch;
  return `<div style="background:${done?'#f9f9f9':'#fff8e1'};border-radius:10px;padding:14px;margin-bottom:10px;border:1px solid ${done?'#ddd':'#ffe082'};">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;">
      <div>
        <div style="font-weight:700;font-size:0.95rem;">${e.type==='packaged'?'📦':'🥬'} ${displayName}</div>
        <div style="font-size:0.8rem;color:var(--muted);">
          ${e.batch?'Batch: '+e.batch+' • ':''}
          ${e.supplier?'Supplier: '+e.supplier+' • ':''}
          ${e.purchaseDate?'Purchase: '+e.purchaseDate+' • ':''}
          Qty: ${e.qty} ${e.unit} ${e.itemId ? '<span style="color:#2e7d32;">(✅ added to Stock)</span>' : '<span style="color:#c62828;">(⚠️ NOT added — no matching Product/Raw Material found for this name)</span>'}
        </div>
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:flex-start;">
        <button onclick="editEmergencyEntry('${e.id}')"
          style="background:#e3f2fd;color:#1565c0;border:1.5px solid #90caf9;padding:6px 12px;border-radius:8px;font-weight:700;cursor:pointer;font-size:0.78rem;">
          ✏️ Edit
        </button>
        ${!done
          ? (awaitingLink
              ? `<span style="background:#e3f2fd;color:#1565c0;padding:6px 12px;border-radius:8px;font-weight:700;font-size:0.78rem;">⏳ Awaiting matching Purchase</span>`
              : `<button onclick="openEndOfDay('${e.id}')"
          style="background:#c62828;color:#fff;border:none;padding:6px 12px;border-radius:8px;font-weight:700;cursor:pointer;font-size:0.82rem;">
          🗑️ Mark Waste
        </button>`)
          : `<span style="color:#2e7d32;font-weight:700;">✅ Done</span>`}
      </div>
    </div>
    ${e.wastedQty ? `<div style="margin-top:8px;font-size:0.82rem;background:#ffebee;border-radius:6px;padding:6px 10px;">
      Wasted: ${e.wastedQty} ${e.unit} → removed from Stock
    </div>` : ''}
  </div>`;
  } catch(_e) { console.error("[SG] renderEmergencyRow:", _e); }
}

async function openEmergencyEntry() {
  try {
  const isManager = currentUser?.role==='Admin' || currentUser?.perms?.includes('Inventory - Emergency Entries');
  ...
  // Reset form
  ['emergencyName','emergencyBatch','emergencySupplier','emergencyTakenBy','emergencyReason','emergencyNewVendorName','emergencyNewVendorContact'].forEach(id=>{
    const el = document.getElementById(id); if(el) el.value='';
  });
  document.getElementById('emergencyEditId').value = '';
  document.getElementById('emergencyModalTitle').textContent = '🚨 New Emergency Entry';
  document.getElementById('emergencySaveBtn').textContent = 'Save Entry';
  ...
  // Populate the Supplier dropdown: Vendors list + Add New + Other (one-time)
  const sel = document.getElementById('emergencySupplierSelect');
  if(sel) {
    const vendors = await getVendors();
    sel.innerHTML = '<option value="">-- Select Supplier --</option>' +
      vendors.map(v => `<option value="${v.name}">${v.name}</option>`).join('') +
      '<option value="__add_new__">+ Add New Vendor</option>' +
      '<option value="__other__">Other (one-time source)</option>';
  }
  document.getElementById('emergencyNewVendorBox').style.display = 'none';
  document.getElementById('emergencyOtherSupplierBox').style.display = 'none';

  document.getElementById('emergencyStockModal').classList.add('open');
  } catch(_e) { console.error("[SG] openEmergencyEntry:", _e); }
}

// Show/hide the right supplier sub-form based on the dropdown choice
function onEmergencySupplierChange() {
  try {
  const val = document.getElementById('emergencySupplierSelect').value;
  document.getElementById('emergencyNewVendorBox').style.display   = (val === '__add_new__') ? 'block' : 'none';
  document.getElementById('emergencyOtherSupplierBox').style.display = (val === '__other__') ? 'block' : 'none';
  } catch(_e) { console.error("[SG] onEmergencySupplierChange:", _e); }
}

// Quick vendor creation from inside the Emergency Entry form
async function quickAddVendorFromEmergency() {
  try {
  const name = document.getElementById('emergencyNewVendorName').value.trim();
  const contact = document.getElementById('emergencyNewVendorContact').value.trim();
  if(!name || !contact) { showToast('Vendor name and contact are required','error'); return; }

  const vendor = {
    id: 'V'+Date.now(), name, company:'', contact, contact2:'', email:'',
    address:'', gst:'', payTerms:'0', rating:'', notes:'',
    createdAt: new Date().toLocaleString('en-IN')
  };
  const vendors = await getVendors();
  vendors.push(vendor);
  setInvData('vendors', vendors);
  await sbUpsertInv('vendors',[vendor]).catch(e=>console.error('quick vendor save fail:',e));

  // Refresh dropdown and select the newly added vendor
  const sel = document.getElementById('emergencySupplierSelect');
  if(sel) {
    sel.innerHTML = '<option value="">-- Select Supplier --</option>' +
      vendors.map(v => `<option value="${v.name}">${v.name}</option>`).join('') +
  ...
  } catch(_e) { console.error("[SG] quickAddVendorFromEmergency:", _e); }
}
...
async function editEmergencyEntry(id) {
  try {
  const entries = await sbGetEmergency();
  const entry = entries.find(e => e.id === id);
  if(!entry) { showToast('Entry not found','error'); return; }

  document.getElementById('emergencyEditId').value = entry.id;
  document.getElementById('emergencyModalTitle').textContent = '✏️ Edit Emergency Entry';
  document.getElementById('emergencySaveBtn').textContent = 'Save Changes';

  document.getElementById('emergencyType').value = entry.type || 'packaged';
  setEmergencyType(entry.type || 'packaged');
  document.getElementById('emergencyName').value = entry.name || '';
  document.getElementById('emergencyBatch').value = entry.batch || '';
  document.getElementById('emergencyQty').value = entry.qty || '';
  document.getElementById('emergencyUnit').value = entry.unit || '';
  document.getElementById('emergencyTakenBy').value = entry.takenBy || '';
  document.getElementById('emergencyReason').value = entry.reason || '';
  document.getElementById('emergencyPurchaseDate').value = entry.purchaseDate || '';
  ...
  if(sel) {
    ...
    sel.innerHTML = '<option value="">-- Select Supplier --</option>' +
      ...
      '<option value="__add_new__">+ Add New Vendor</option>' +
      '<option value="__other__">Other (one-time source)</option>';
    const vendorMatch = vendors.find(v => v.name === entry.supplier);
    if(vendorMatch) {
      sel.value = entry.supplier;
      document.getElementById('emergencyOtherSupplierBox').style.display = 'none';
    } else if(entry.supplier) {
      sel.value = '__other__';
      document.getElementById('emergencySupplier').value = entry.supplier;
      document.getElementById('emergencyOtherSupplierBox').style.display = 'block';
    }
  }
  document.getElementById('emergencyNewVendorBox').style.display = 'none';
  ...
  document.getElementById('emergencyStockModal').classList.add('open');
  } catch(_e) { console.error("[SG] editEmergencyEntry:", _e); }
}

async function saveEmergencyEntryEdit(editId) {
  try {
  const type = document.getElementById('emergencyType').value;
  const name = document.getElementById('emergencyName').value.trim();
  const batch = document.getElementById('emergencyBatch').value.trim();
  const supplierSelVal = document.getElementById('emergencySupplierSelect')?.value || '';
  const supplier = (supplierSelVal === '__other__')
    ? document.getElementById('emergencySupplier').value.trim()
    : (supplierSelVal === '__add_new__' ? '' : supplierSelVal);
  const purchaseDate = document.getElementById('emergencyPurchaseDate').value;
  const qty = parseFloat(document.getElementById('emergencyQty').value) || 0;
  ...
  const oldQty       = parseFloat(entry.qty) || 0;
  const oldItemId    = entry.itemId || null;
  const oldStockType = entry.stockType || null;

  // Resolve the (possibly changed) matching product/rawmat by the (possibly edited) name
  const products = await getProducts();
  const rawmats  = await getRawmats();
  const nameLC = name.toLowerCase();
  let matched = products.find(p => (p.name||'').toLowerCase() === nameLC);
  let itemType = matched ? (matched.type || 'company') : null;
  if(!matched) {
    matched = rawmats.find(r => (r.name||'').toLowerCase() === nameLC);
    itemType = matched ? 'rawmat' : null;
  }

  // Reverse whatever stock this entry previously added, before re-adding under
  // the (possibly different) resolved item and (possibly different) qty
  if(oldItemId && oldStockType) {
    await updateStock(oldStockType, oldItemId, oldQty, 'remove');
  }

  let stockAdded = false;
  if(matched) {
    await updateStock(itemType, matched.id, qty, 'add');
    stockAdded = true;
  }

  Object.assign(entry, {
    type, name, batch, supplier, purchaseDate, qty, unit, takenBy, reason,
    itemId:    matched ? matched.id : null,
    stockType: matched ? itemType   : null,
    editedAt:  new Date().toLocaleString('en-IN'),
    editedBy:  currentUser?.name || 'Admin'
  });

  await sbUpsertInv('sg_emergency_stock', [entry]).catch(e => console.error('emergency edit save fail:', e));

  // Keep the matching Track Record row in sync (preserve any existing Purchase link)
  try {
    const consumptions = await getConsumptions();
    const trackId = 'EC_' + entry.id;
    const idx = consumptions.findIndex(c => c.id === trackId);
    const recordData = {
      id:         trackId,
      date:       entry.purchaseDate || new Date().toISOString().slice(0,10),
      time:       (entry.autoDateTime||'').replace(/.*?(\d{1,2}:\d{2}\s*[ap]m).*/i,'$1').trim() || new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}),
      type:       matched ? itemType : (type || 'rawmat'),
      itemName:   entry.name,
      ...
      purpose:    entry.reason   || 'Emergency Usage',
    ...
    if(idx >= 0) {
      recordData.purchaseId = consumptions[idx].purchaseId || '';
      consumptions[idx] = { ...consumptions[idx], ...recordData };
    } else {
      recordData.purchaseId = '';
      consumptions.unshift(recordData);
    }
    setInvData('consumptions', consumptions);
  } catch(_c) { console.error('[SG] emergency edit track-record update fail:', _c); }

  invInvalidate('emergency');
  closeModal('emergencyStockModal');
  renderEmergencyStock();
  showToast(stockAdded
    ? '✅ Entry updated — Stock adjusted!'
    : '✅ Entry updated — but no matching Product/Raw Material named "'+name+'" was found, so Stock was NOT adjusted.');
  } catch(_e) { console.error("[SG] saveEmergencyEntryEdit:", _e); }
}

async function saveEmergencyEntry() {
  try {
  const editId = document.getElementById('emergencyEditId')?.value;
  if(editId) { await saveEmergencyEntryEdit(editId); return; }
  const type = document.getElementById('emergencyType').value;
  const name = document.getElementById('emergencyName').value.trim();
  const batch = document.getElementById('emergencyBatch').value.trim();
  const supplierSelVal = document.getElementById('emergencySupplierSelect')?.value || '';
  ...
  // Immediately add this qty to Main Stock — it's already physically in the kitchen.
  // (When a matching formal Purchase is entered later, its auto-link correction
  // subtracts this same qty back out, so it doesn't get double-counted.)
  let stockAdded = false;
  try {
    const products = await getProducts();
    const rawmats  = await getRawmats();
    ...
    if(matched) {
      await updateStock(itemType, matched.id, entry.qty, 'add');
      entry.itemId = matched.id;
      entry.stockType = itemType;
      await sbUpsertInv('sg_emergency_stock',[entry]).catch(e=>console.error('emergency itemId save fail:',e));
      stockAdded = true;
    }

    // Push to Track Records Usage immediately too — so the next shift can
    // see exactly what got taken as Emergency, right away (no Purchase ID
    // yet — that gets filled in automatically once a matching Purchase is entered).
    try {
      const consumptions = await getConsumptions();
      const now = new Date();
      consumptions.unshift({
        ...
        time:       (entry.autoDateTime||'').replace(/.*?(\d{1,2}:\d{2}\s*[ap]m).*/i,'$1').trim() || now.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}),
        type:       matched ? itemType : (type || 'rawmat'),
        itemName:   entry.name,
        itemId:     matched ? matched.id : '',
        qty:        entry.qty,
      ...
      setInvData('consumptions', consumptions);
    } catch(_c) { console.error('[SG] emergency track-records push fail:', _c); }
  } catch(_s) { console.error('[SG] emergency stock-add fail:', _s); }

  invInvalidate('emergency');
  closeModal('emergencyStockModal');
  renderEmergencyStock();
  showToast(stockAdded
    ? '✅ Emergency entry saved — added to Main Stock!'
    : '✅ Emergency entry saved — but no matching Product/Raw Material named "'+name+'" was found, so Main Stock was NOT updated. Please add it there manually.');
...
async function openEndOfDay(entryId) {
  ...
  if(!entry) return;
  if(!entry.name && entry.batch) {
    showToast('⚠️ This entry only has a Batch/HRN (no item name yet). It will auto-link once a matching Purchase with the same HRN is entered.', 'error');
    return;
  }

  // Stock was already added in full when this entry was saved. "Used" now happens
  // automatically via the normal recipe/order deduction flow — nothing manual
  // needed for that. This step is only for marking any portion that spoiled/wasted.
  const wastedQty = prompt(`Mark Waste for: ${entry.name}\nTotal Qty Added to Stock: ${entry.qty} ${entry.unit}\n\nQuantity WASTED / spoiled (removed from Stock):`);
  if(wastedQty === null) return;

  const wasted = parseFloat(wastedQty)||0;
  if(wasted <= 0) { showToast('Enter a wasted quantity greater than 0 (or cancel if nothing was wasted)','error'); return; }
  if(wasted > entry.qty) { showToast('Wasted cannot exceed the total quantity','error'); return; }
  ...
  // Remove the wasted qty from Main Stock (it was added in full at save-time)
  if(entry.itemId && entry.stockType) {
    await updateStock(entry.stockType, entry.itemId, wasted, 'remove');
  } else {
    const products = await getProducts();
    const rawmats  = await getRawmats();
    const entryNameLC = (entry.name||'').toLowerCase().trim();
    let matched = products.find(p => (p.name||'').toLowerCase().trim() === entryNameLC);
    let itemType = matched ? (matched.type || 'company') : null;
    if(!matched) {
      matched = rawmats.find(r => (r.name||'').toLowerCase().trim() === entryNameLC);
      itemType = matched ? 'rawmat' : null;
    }
    if(matched) await updateStock(itemType, matched.id, wasted, 'remove');
    else {
      console.warn('[SG] openEndOfDay: could not resolve stock item for', entry.name);
      showToast('⚠️ Marked as wasted, but no matching stock item found — Main Stock not adjusted. Please adjust it manually.','error');
    }
  }
  ...
  renderEmergencyStock();
  showToast('✅ Marked ' + wasted + ' ' + entry.unit + ' as wasted — removed from Stock.');
...
async function analyseEmergencyImport() {
  ...
  const entries = await sbGetEmergency();
  const pending = entries.filter(e=>!e.reconciled);
  const purchases = await getPurchases(); // real purchases (was incorrectly using a non-existent table before)
  const div = document.getElementById('importAnalysisResult');
  if(!div) return;
  ...
  pending.forEach(entry => {
    // Find a matching purchase — by HRN, or by item name + supplier + date
    let matchedPurchase = null;
    if(entry.batch) {
      matchedPurchase = purchases.find(p => (p.batch||'').toLowerCase() === (entry.batch||'').toLowerCase());
    } else {
      matchedPurchase = purchases.find(p =>
        (p.itemName||'').toLowerCase().trim() === (entry.name||'').toLowerCase().trim() &&
        (p.vendorName||'').toLowerCase().trim() === (entry.supplier||'').toLowerCase().trim() &&
    ...
    const nameMatch = matchedPurchase ? true : null;
    const hasIssue = !matchedPurchase;
    ...
    html += `<div style="background:${hasIssue?'#fce4ec':'#e8f5e9'};border-radius:10px;padding:14px;margin-bottom:10px;border:1px solid ${hasIssue?'#ef9a9a':'#a5d6a7'};">
      <div style="font-weight:700;margin-bottom:8px;">${hasIssue?'⚠️':'✅'} ${entry.name||'(unnamed)'} — ${entry.qty} ${entry.unit}</div>
      <div style="font-size:0.82rem;line-height:1.8;">
        <div>Match: ${entry.batch?'Batch: '+entry.batch:'Supplier: '+entry.supplier+' | Purchase: '+entry.purchaseDate}
          → ${matchedPurchase?'✅ Found — Purchase #'+matchedPurchase.id:'❌ No matching Purchase found yet'}</div>
        <div>Entry logged: ${entryDate||'—'} ${entryTime||''}</div>
      </div>
      ${hasIssue?`<div style="font-size:0.8rem;color:#c62828;margin-top:6px;font-weight:600;">
        ⚠️ No Purchase entered yet for this item — nothing to link until one is added</div>`:''}
    </div>`;
  });

  div.innerHTML = html + `<div style="margin-top:16px;display:flex;gap:10px;">
    <button onclick="confirmImportAll()" style="flex:1;background:#2e7d32;color:#fff;border:none;padding:10px;border-radius:8px;font-weight:700;cursor:pointer;">
      🔗 Link All Matched Now
    </button>
  </div>`;
  } catch(_e) { console.error("[SG] analyseEmergencyImport:", _e); }
}

// Manually (re)trigger the same reconciliation that normally runs automatically
// when a Purchase is saved — useful if that auto-link ever gets missed. Does
// NOT touch Stock (Emergency Entries already add their qty to Stock immediately
// when saved) — it only links matching Purchases and resolves any "⏳ Pending"
// cost entries in the Recipe Consumption Log.
async function confirmImportAll() {
  ...
  const entries = await sbGetEmergency();
  const pending = entries.filter(e=>!e.reconciled);
  if(!pending.length) { showToast('Nothing pending to link','error'); return; }

  const purchases = await getPurchases();
  if(!purchases.length) { showToast('No purchases found to match against','error'); return; }

  const result = await reconcileEmergencyAndPendingCost(purchases);

  invInvalidate('emergency');
  renderEmergencyStock();
  await analyseEmergencyImport();

  if(result.totalLinked > 0) {
    showToast('🔗 ' + result.totalLinked + ' emergency entr' + (result.totalLinked>1?'ies':'y') + ' linked to matching purchases!');
  } else {
    showToast('No new matches found — those items still need a matching Purchase entered first.','error');
  }
...
const DEFAULT_COLUMNS = {
  orders: [
    ...
    {id:'source', label:'Source', field:'source', removable:true},
...
function getColumns(section) {
  ...
  // One-time migration: inject "Source" column (Website/Phone/Walk-In/etc) for
  // existing orders configs that predate this feature
  if(section === 'orders' && !cols.some(c => c.field === 'source')) {
    const itemsIdx = cols.findIndex(c => c.field === 'items');
    const insertAt = itemsIdx >= 0 ? itemsIdx + 1 : Math.max(0, cols.length - 1);
    cols.splice(insertAt, 0, {id:'source', label:'Source', field:'source', removable:true});
    saveColumns(section, cols);
  }
  // One-time migration: the "Per Weight" column used to be saved with the
  // wrong field name ("perWeight" instead of "unitWt") — purchases never
  // actually had a .perWeight property, so this column always showed blank
  // even though the real Wt/Vol-per-unit data existed under .unitWt.
  if(section === 'purchases') {
    let fixedAny = false;
    cols.forEach(c => {
      if(c.field === 'perWeight') { c.field = 'unitWt'; fixedAny = true; }
    });
    if(fixedAny) saveColumns(section, cols);
  }
  return cols;
}
