...
function calcEmpStats(staffId, range) {
  ...
  // Bug #3 fix: nested getOrderDate (jo created_at ko UTC substring karta tha aur
  // raat ke IST orders ko galat din count karta tha) hata diya.
  // Ab calls global getOrderDate() -> parseAnyDate() pe resolve hote hain, jo
  // 'time' (IST locale) ko priority deta hai aur created_at ko UTC->IST convert karta hai.
...
function renderTaskLoad() {
  ...
  const loads = staffList.map(s=>{
    ...
    const myOrders = allOrders.filter(o=>{
      ...
      // Bug #8 fix: pehle (o.time).substring(0,10) "15/7/2026," jaisa garbage deta tha
      // jo YYYY-MM-DD range se kabhi match nahi hota tha — local orders drop ho jaate the.
      // Ab wahi canonical getOrderDate() use hota hai jo baaki performance code karta hai.
      const orderDate = getOrderDate(o);
      return isAssigned && orderDate >= range.from && orderDate <= range.to;
    });
...
function switchConsumTab(tab) {
  ...
  // Dish filter — sirf recipe tab pe dikhao
...
async function renderIngredientTotalConsumption() {
  try {
    ...
    if(error || !data || !data.length) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:20px;color:var(--muted);">Koi data nahi mila</td></tr>';
      return;
    }
    ...
    if(fromDate && toDate && new Date(fromDate) > new Date(toDate)) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:20px;color:#e74c3c;">⚠️ Start date, end date se baad ki hai — date range theek karo</td></tr>';
    ...
    if(!filtered.length) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;padding:20px;color:var(--muted);">Is date range mein koi record nahi mila</td></tr>';
      const summary = document.getElementById('ingredTotalSummary');
      if(summary) summary.style.display = 'none';
      return;
    }
...
async function renderConsumptionLog() {
  ...
  if(!data || !data.length) {
    tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;padding:20px;color:var(--muted);">Koi consumption record nahi mila. (data length: 0)</td></tr>';
    return;
  }
  ...
  // Date range validation
  if(fromDate && toDate && new Date(fromDate) > new Date(toDate)) {
    tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;padding:20px;color:#e74c3c;">⚠️ Start date, end date se baad ki hai — date range theek karo</td></tr>';
  ...
  if(!filtered.length) {
    tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;padding:20px;color:var(--muted);">Filter mein koi record nahi mila</td></tr>';
    const summary = document.getElementById('consumLogSummary');
    if(summary) summary.style.display = 'none';
    return;
  }
...
async function getIngredientCost(itemId) {
  ...
  // basePricePerUnit already per gram/ml hai
...
// ── FIFO Cost Helper ─────────────────────────────────────────
// Supabase se item ki purchases fetch karo FIFO order mein
async function getFIFOCost(itemId, neededQty) {
  try {
  // Supabase se purchases fetch karo FIFO order mein
...
async function autoDeductStockForOrder(orderItems, orderId, servedBy) {
  ...
  // Duplicate check — agar orderId already consume hua hai toh skip
  try {
  ...
  // Menu items se name → id mapping banao
...
async function renderExpiryProducts() {
  try {
    ...
    if(!rows.length) {
      tbody.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:20px;color:var(--muted);">Koi future expiry product nahi mila</td></tr>';
      return;
    }
...
async function renderExpiryRawmat() {
  try {
    ...
    if(!rows.length) {
      tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:20px;color:var(--muted);">Koi future expiry raw material nahi mila</td></tr>';
      return;
    }
...
async function renderExpiredPending() {
  try {
    ...
    if(!rows.length) {
      tbody.innerHTML = '<tr><td colspan="11" style="text-align:center;padding:24px;color:#2e7d32;font-weight:600;">✅ Koi expired raw material pending nahi hai</td></tr>';
      return;
    }
...
async function openWasteFromExpired(encodedP) {
  try {
    ...
    if(wItem) {
      ...
      // Agar option nahi mila toh manually add karo
    ...
    // Modal open - openModal use karo taaki closeModal kaam kare
    openModal('invWasteModal');

    // Focus qty
    if(wQty) wQty.focus();

    showToast('✅ Waste form ready — sirf quantity bharo');
...
async function renderEmergencyStock() {
  ...
  if(!div) return;
  const isManager = currentUser?.role==='Admin' || currentUser?.perms?.includes('Inventory - Emergency Stock');
  if(!isManager) {
...
(Remaining: ${entry.qty - parseFloat(usedQty||0)} ${entry.unit})`);
  ...
  if(wasted > 0) {
    const wasteEntry = { id:'W_'+Date.now(), item:entry.name, qty:wasted, unit:entry.unit, reason:'Emergency Stock - '+entry.reason, date:new Date().toLocaleDateString('en-IN'), by:currentUser?.name||'Manager' };
...
async function importEntriesToStock(toImport, allEntries) {
  ...
  // ✅ Purchase import complete hone ke BAAD hi Track Records Usage mein push karo
  try {
    const now = new Date();
    ...
    toImport.forEach(entry => {
      ...
      if(existingIds.has(trackId)) return; // already pushed — duplicate avoid karo
    ...
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:560px;width:90%;max-height:90vh;overflow-y:auto;">
      ...
      <div id="colList" style="margin-bottom:16px;">
        ${cols.map((c,i) => `
          <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:#f9f9f9;border-radius:8px;margin-bottom:6px;border:1px solid #eee;">
            ...
            <button onclick="editColDescription('${section}',${i})" title="${c.description ? 'Note: '+c.description.replace(/"/g,'&quot;') : 'Client note add karo'}"
      ...
      <div style="background:#e8f5e9;border-radius:10px;padding:14px;margin-bottom:16px;">
        ...
        <div id="newColCustomKeyWrap" style="display:none;margin-top:8px;">
          <input type="text" id="newColCustomKey" placeholder="Naya field ka naam (e.g. zone, table_pref)"
            style="width:100%;padding:8px;border:2px solid #2e7d32;border-radius:6px;font-family:inherit;box-sizing:border-box;">
          <div style="font-size:0.72rem;color:#2e7d32;margin-top:3px;margin-bottom:8px;">Yeh column har order row mein editable text box ke roop mein dikhega.</div>
          <label style="font-size:0.8rem;font-weight:700;display:block;margin-bottom:5px;color:#333;">💬 Client Note (optional)</label>
          <textarea id="newColDescription" rows="3" placeholder="Yahan likho ki is column mein exactly kya dikhana/karna hai... (e.g. 'Yahan customer ka birthday date daalna hai taki special offer bhej sakein')"
          ...
          <div style="font-size:0.7rem;color:#1565c0;margin-top:3px;background:#e3f2fd;padding:6px 8px;border-radius:6px;">
            🤖 <b>Smart:</b> Agar likha ho "size", "address", "mobile", "status", "total" jaise words — to system khud asli data dhoond kar dikha dega (manual entry ki zaroorat nahi)! Bilkul naya/manual field ho to bhi yeh note hover par dikhega.
          </div>
        </div>
...
function editColDescription(section, idx) {
  ...
  const c = cols[idx];
  if(!c) return;
  const note = prompt('💬 "'+c.label+'" column ke liye client note likho — yahan likho ki exactly kya dikhna/save hona chahiye:', c.description||'');
...
function addNewCol(section) {
  ...
  if(mappedField === 'custom') {
    ...
    if(!key) {
      showToast('Naya field ka naam likho!','error');
      if(keyInp) keyInp.focus();
      return;
    }
  ...
  dialog.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;max-width:420px;width:90%;">
      ...
      <div id="unknownCustomKeyWrap" style="display:none;margin-bottom:16px;">
        <input type="text" id="unknownCustomKey" placeholder="Naya field ka naam (e.g. zone, table_pref)"
...
function confirmUnknownField(label, section) {
  ...
  if(field === 'custom') {
    ...
    const key = keyInp ? keyInp.value.trim() : '';
    if(!key) { showToast('Naya field ka naam likho!','error'); if(keyInp) keyInp.focus(); return; }
...
function updateTableHeaders() {
  try {
    ...
    // Orders — S.No. sticky column hamesha pehle, phir baaki columns
    ...
    // Customers — S.No. sticky column hamesha pehle
...
function renderKPI() {
  ...
  const stock = _invCache.stock || [];
  const waste = _invCache.waste || [];
  ...
  const wasteValue = wasteItems.reduce((s,w)=>s+(parseFloat(w.totalCost)||0),0);

  // Low stock count
  const lowStock = stock.filter(s=>parseFloat(s.quantity||0)<=parseFloat(s.minStock||5)).length;
  ...
  // Waste %
  const totalStockValue = stock.reduce((s,x)=>s+(parseFloat(x.quantity||0)*parseFloat(x.price||0)),0);
  const wastePct = totalStockValue > 0 ? Math.round((wasteValue/totalStockValue)*100) : 0;
  ...
  if(invEl) {
    const lowItems = stock.filter(s=>parseFloat(s.quantity||0)<=parseFloat(s.minStock||5));
    invEl.innerHTML = lowItems.length ?
      '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;">'+
        lowItems.map(s=>'<div style="background:#fff8e1;border-radius:8px;padding:10px;border:1px solid #ffe082;">'+
          '<div style="font-weight:700;font-size:0.85rem;">⚠️ '+s.name+'</div>'+
          '<div style="font-size:0.75rem;color:#e65100;">'+s.quantity+' '+s.unit+' left</div>'+
        '</div>').join('')+
      '</div>' :
...
function renderAutoReport(){
  ...
  var waste=(_invCache.waste||[]).filter(function(w){return (w.date||'')>=from&&(w.date||'')<=to;});
  ...
  var wasteVal=waste.reduce(function(s,w){return s+(parseFloat(w.totalCost)||0);},0);
...
function injectCSVData() {
  ...
  showToast('✅ 30 purchases + 18 stock items load ho gaye!');
